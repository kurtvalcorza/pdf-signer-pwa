...
- [X] T050 [P] Performance pass — **BENCHMARKED** (`npm run perf`, `tests/e2e-perf/perf.spec.ts`, production build + Pixel 7 + 4× CPU throttle). Results (see [docs/perf.md](../../docs/perf.md)): 60-page PDF open+first render **301 ms**; placement drag **60.4 fps** (meets the 60 fps goal — no dropped frames/main-thread block); sign+download **421 ms** (goal ~2 s); full open→place→sign→download **977 ms** vs **SC-001's 60 s budget**. All goals met with large margins. Not in CI (timings too noisy to gate). Caveats documented: 4× throttle ≈ but ≠ a real mid-range device; image-heavy/multi-MB PDFs unmeasured
