...
| V. Verify Against Real Readers (Test-First) | pyHanko/`pdfsig` validation gate + Acrobat spot-check; TDD | ✅ **Fully honored — CI wired.** `.github/workflows/ci.yml` runs build + tests + `npm run verify:signatures` (pyHanko) on every push to `main` and every PR, plus the Playwright/PWA E2E. The earlier "deferred to a local pre-push hook" deviation is CLOSED (that hook was never actually implemented — the gate was honor-system until now). Acrobat remains a manual per-release spot-check |
